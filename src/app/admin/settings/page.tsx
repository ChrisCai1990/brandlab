  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <div className="bg-white border border-[#95d5b2] rounded-2xl p-6 mb-5">
        <h2 className="text-sm font-bold text-[#1b4332] mb-5">修改管理员密码</h2>
        <form onSubmit={handleChangePassword} className="space-y-4">
          ...
          {status !== "idle" && (
            <p className={`text-xs px-3 py-2 rounded-lg ${status === "success" ? "bg-green-50 text-green-600" : status === "error" ? "bg-red-50 text-red-500" : "text-[#6b7280]"}`}>
              {status === "loading" ? "保存中..." : msg}
            </p>
          )}

          <button type="submit" disabled={status === "loading"} className="bg-[#1b4332] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#40916c] transition-colors disabled:opacity-40">
            保存修改
          </button>
        </form>
      </div>
