...
const CONTENT_TYPES: Record<string, { bg: string; text: string }> = {
  "个人定位": { bg: "bg-[#0a0a0a]", text: "text-[#a0a0a0]" },
    ...
    <div className="bg-black min-h-screen">
      ...
      <div className="border-b border-[#1f1f1f] bg-[#0a0a0a]">
        <div className="max-w-5xl mx-auto px-8 py-3 flex items-center gap-2 text-xs text-[#888888]">
          ...
          <span>/</span>
          <span className="text-white font-medium">30天内容排期生成器</span>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-bold text-white mb-3">AI 30天内容排期生成器</h1>
          <p className="text-sm text-[#888888]">输入账号方向，Claude AI 自动生成一份30天专属内容计划，涵盖选题标题 + 内容角度</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full border border-[#1f1f1f] rounded-none px-4 py-3 text-sm text-white placeholder-[#333333] focus:outline-none focus:border-[#333333] bg-black"
              />
              <p className="text-[10px] text-[#888888] mt-1.5">越具体越好，生成的选题更精准</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-[#888888] tracking-widest uppercase mb-3">
                目标受众（可选）
              ...
              <div className="flex flex-wrap gap-2">
                {PLATFORMS.map((p) => (
                  <button key={p} onClick={() => setPlatform(p)}
                    className={`text-xs px-4 py-2 rounded-sm border transition-all ${platform === p ? "border-[#333333] bg-black text-[#a0a0a0] font-medium" : "border-[#1f1f1f] text-[#888888] hover:border-[#333333]"}`}>
                    {p}
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {FREQ_OPTIONS.map((opt) => (
                  <button key={opt.value} onClick={() => setFrequency(opt.value)}
                    className={`border rounded-none py-3 px-2 text-center transition-all ${
                      frequency === opt.value
                        ? "border-[#333333] bg-black text-[#a0a0a0]"
                        : "border-[#1f1f1f] text-[#888888] hover:border-[#333333]"
                    }`}>
        ...
        {generated && (
          <>
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-bold text-white">你的30天内容计划</h2>
            ...
            <div className="grid grid-cols-7 gap-2 mb-2">
              {WEEK_LABELS.map((l) => (
                <div key={l} className="text-center text-[10px] text-[#888888] font-medium">周{l}</div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-2">
              {grid.map((cell) => {
                ...
                return (
                  <div key={cell.day}
                    className={`rounded-none p-3 min-h-[128px] flex flex-col ${
                      cell.active && cell.item
                        ? "border border-[#1f1f1f] hover:border-[#333333] hover:shadow-sm transition-all bg-black"
                        : cell.active
                        ? "border border-[#1f1f1f] bg-black"
                        : "bg-[#0a0a0a] border border-dashed border-[#E0EBE5]"
                    }`}
                  >
                    <div className={`text-xs font-bold mb-2 ${cell.active ? "text-white" : "text-[#555555]"}`}>
                      {cell.day}
                    </div>
                    {cell.active && cell.item && typeStyle ? (
                      <>
                        <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-sm self-start mb-1.5 ${typeStyle.bg} ${typeStyle.text}`}>
                          {cell.item.type}
                        </span>
                        <span className="text-[9px] text-[#888888] border border-[#E0EBE5] px-1.5 py-0.5 rounded self-start mb-1.5">
                          {cell.item.hook}
                        </span>
                        <p className="text-[10px] text-[#888888] leading-snug line-clamp-3">
                          {cell.item.title}
                        </p>
                      </>
                    ) : cell.active ? (
                      <span className="text-[9px] text-[#555555] mt-auto">待填充</span>
                    ) : (
                      <span className="text-[9px] text-[#555555] mt-auto">休息</span>
                    )}
                  </div>
                );
              })}
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {CONTENT_TYPE_LIST.map((label) => {
                ...
                return (
                  <span key={label} className={`text-[10px] px-2.5 py-1 rounded-sm font-medium ${s.bg} ${s.text}`}>
                    {label}
                  </span>
                );
              })}
            ...
            {/* Detail list */}
            <div className="mt-10">
              <h3 className="text-sm font-bold text-white mb-4">选题详情</h3>
              <div className="space-y-2">
                {grid.filter((c) => c.active && c.item).map((cell) => {
                  ...
                  return (
                    <div key={cell.day} className="border border-[#1f1f1f] rounded-none px-5 py-4 flex items-start gap-4">
                      <div className="shrink-0 text-xs font-bold text-[#888888] w-10">第{cell.day}天</div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-sm ${typeStyle.bg} ${typeStyle.text}`}>
                          {cell.item!.type}
                        </span>
                        <span className="text-[10px] text-[#888888] border border-[#E0EBE5] px-2 py-0.5 rounded">
                          {cell.item!.hook}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-white">{cell.item!.title}</p>
                        {cell.item!.angle && (
                          <p className="text-[11px] text-[#888888] mt-1">{cell.item!.angle}</p>
                        )}
                      </div>
                    </div>
                  );
            ...
            {extras.length > 0 && (
              <div className="mt-8">
                <h3 className="text-sm font-bold text-white mb-3">3条备用选题</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {extras.map((ex, i) => {
                    const typeStyle = CONTENT_TYPES[ex.type] || { bg: "bg-slate-100", text: "text-slate-500" };
                    return (
                      <div key={i} className="border border-[#1f1f1f] rounded-none p-4">
                        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-sm ${typeStyle.bg} ${typeStyle.text} mb-2 inline-block`}>
                          {ex.type}
                        </span>
            ...
            {/* CTA */}
            <div className="mt-10 bg-[#0a0a0a] rounded-none p-6 flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                <p className="text-sm font-bold text-white mb-1">需要更多定制化支持？</p>
                <p className="text-xs text-[#a0a0a0]">加入社群，获取 Notion 版本 + 内容框架模板</p>
              </div>
              <Link href="/contact"
                className="shrink-0 text-xs bg-white border border-[#333333] text-white px-5 py-2.5 rounded-none hover:bg-[#111111] transition-colors">
                加入社群
              </Link>
            </div>
          </>
