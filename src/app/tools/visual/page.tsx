...
const STYLE_OPTIONS = [
  { id: "minimal", label: "极简知性", desc: "大量留白，线条干净，专业感强", colors: ["#1A2E22", "#6BAF8A", "#F7FBF8", "#E8F5EE"] },
...
const STYLE_SYSTEMS: Record<string, {
  ...
  minimal: {
    headline: "极简知性 · 专业沉稳",
    palette: [
      { name: "深绿", hex: "#1A2E22", role: "主色·文字·背景块" },
      { name: "清绿", hex: "#6BAF8A", role: "强调色·标签·图标" },
      { name: "奶白", hex: "#F7FBF8", role: "页面背景" },
      { name: "浅绿", hex: "#E8F5EE", role: "卡片背景·分割区" },
    ],
    ...
    <div className="bg-white min-h-screen">
      <div className="border-b border-[#C8DDD2] bg-[#F7FBF8]">
        <div className="max-w-5xl mx-auto px-8 py-3 flex items-center gap-2 text-xs text-[#6B7A6E]">
          ...
          <span>/</span>
          <span className="text-[#1A2E22] font-medium">视觉系统设计模板</span>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-bold text-[#1A2E22] mb-3">视觉系统设计模板</h1>
          <p className="text-sm text-[#6B7A6E]">选择你的账号风格，生成配色方案 + 字体规范 + 封面设计指南，建立统一的视觉识别系统</p>
        </div>
        ...
        <div className="bg-[#F7FBF8] border border-[#C8DDD2] rounded-2xl p-8 mb-10 space-y-7">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {STYLE_OPTIONS.map((s) => (
                <button
                  ...
                  className={`border rounded-xl p-4 text-left transition-all ${form.style === s.id ? "border-[#2D6A4F] bg-white shadow-sm" : "border-[#C8DDD2] hover:border-[#6BAF8A] bg-white"}`}
                >
                  ...
                  <p className={`text-xs font-bold mb-0.5 ${form.style === s.id ? "text-[#2D6A4F]" : "text-[#1A2E22]"}`}>{s.label}</p>
                  <p className="text-[10px] text-[#6B7A6E] leading-snug">{s.desc}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-xs font-medium text-[#6BAF8A] tracking-widest uppercase mb-3">目标平台</label>
              <div className="flex flex-wrap gap-2">
                {PLATFORM_OPTIONS.map((p) => (
                  <button key={p} onClick={() => setForm(f => ({ ...f, platform: p }))}
                    className={`text-xs px-4 py-2 rounded-full border transition-all ${form.platform === p ? "border-[#2D6A4F] bg-[#E8F5EE] text-[#2D6A4F] font-medium" : "border-[#C8DDD2] text-[#6B7A6E] hover:border-[#6BAF8A]"}`}>
                    {p}
                  </button>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-medium text-[#6BAF8A] tracking-widest uppercase mb-3">内容调性</label>
              <div className="flex flex-wrap gap-2">
                {TONE_OPTIONS.map((t) => (
                  <button key={t} onClick={() => setForm(f => ({ ...f, tone: t }))}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-all ${form.tone === t ? "border-[#2D6A4F] bg-[#E8F5EE] text-[#2D6A4F] font-medium" : "border-[#C8DDD2] text-[#6B7A6E] hover:border-[#6BAF8A]"}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-xs font-medium text-[#6BAF8A] tracking-widest uppercase mb-2">账号名/品牌名（可选）</label>
              <input
                type="text"
                value={form.nickname}
        ...
        {result && (
          <>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-lg font-bold text-[#1A2E22]">你的视觉系统规范</h2>
                <p className="text-xs text-[#6B7A6E] mt-1">{result.headline}</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="border border-[#C8DDD2] rounded-xl p-5">
                <p className="text-xs text-[#6BAF8A] font-medium tracking-widest uppercase mb-4">主色系方案</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {result.palette.map((p) => (
                    <div key={p.hex}>
                      ...
                      />
                      <p className="text-xs font-bold text-[#1A2E22]">{p.name}</p>
                      <p className="text-[10px] text-[#6B7A6E] font-mono">{p.hex}</p>
                      <p className="text-[10px] text-[#6B7A6E] leading-snug mt-0.5">{p.role}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* 模拟封面预览 */}
              <div className="border border-[#C8DDD2] rounded-xl p-5">
                <p className="text-xs text-[#6BAF8A] font-medium tracking-widest uppercase mb-4">封面效果预览</p>
                <div className="grid grid-cols-3 gap-3">
                  {result.cover.map((template, i) => {
                    ...
                    return (
                      <div key={i} className="aspect-[3/4] rounded-xl overflow-hidden border border-[#C8DDD2] relative flex flex-col" style={{ backgroundColor: bgColor }}>
                        {i === 0 && (
                          <>
                        ...
                        {i === 1 && (
                          <div className="flex-1 flex flex-col items-center justify-center p-4">
                            ...
                            <div className="mt-4 px-4 py-1.5 rounded-full" style={{ backgroundColor: accentColor }}>
                ...
                <div className="mt-3 space-y-1.5">
                  {result.cover.map((c, i) => (
                    <p key={i} className="text-[11px] text-[#6B7A6E]">
                      <span className="font-medium text-[#1A2E22]">方案{i + 1}：</span>{c}
                    </p>
                  ))}
                </div>
              </div>
              ...
              {/* 字体系统 */}
              <div className="border border-[#C8DDD2] rounded-xl p-5">
                <p className="text-xs text-[#6BAF8A] font-medium tracking-widest uppercase mb-4">字体系统</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <p className="text-[10px] text-[#6BAF8A] font-medium mb-1">中文字体</p>
                    <p className="text-xs text-[#1A2E22] font-medium">{result.fonts.cn}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-[#6BAF8A] font-medium mb-1">英文辅助字体</p>
                    <p className="text-xs text-[#1A2E22] font-medium">{result.fonts.en}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-[#6BAF8A] font-medium mb-1">字号规范</p>
              ...
              {/* 设计原则 */}
              <div className="border border-[#C8DDD2] rounded-xl p-5">
                <p className="text-xs text-[#6BAF8A] font-medium tracking-widest uppercase mb-4">视觉设计原则</p>
                <div className="space-y-2.5">
                  {result.tips.map((tip, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <span className="text-xs font-bold text-[#6BAF8A] shrink-0 w-5">{String(i + 1).padStart(2, "0")}</span>
            ...
            <div className="mt-8 bg-[#1A2E22] rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-[#A8D5BB]">加入社群，获取4套可直接编辑的封面模板 + 视觉规范完整文档</p>
              </div>
        ...
        <div className="mt-16 border-t border-[#C8DDD2] pt-14">
          <div className="mb-6">
            ...
            <h2 className="text-xl font-bold text-[#1A2E22] mb-2">用 AI 设计更专业的视觉系统</h2>
            <p className="text-sm text-[#6B7A6E]">把下方提示词复制到 ChatGPT 或 Claude，详细描述你的账号定位，获得完整定制的视觉系统设计方案。</p>
          </div>
